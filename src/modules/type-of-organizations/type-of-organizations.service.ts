...
export class TypeOfOrganizationsService implements OnModuleInit {
    ...
    async findByName(name: string): Promise<TypeOfOrganization | null> {
    ...
    async findOne(id: string): Promise<TypeOfOrganization | null> {
        return await this.typeOfOrganizationRepository.findOne({
            where: { id },
        });
    }
}
